...
```evalbc
push_i 7          ; argument (y)
push_a $clo_foo   ; $value_foo refers to the closure
...
Normally, a call pushes the _next_ PC onto the stack so that control resumes from the instruction
_after_ the call. For an icall when there are too many given arguments, we actually need to
_repeat_ the calling logic a number of times bounded by the count of given arguments. Thus, an
icall pushes _its own address_ onto the stack. So how do we know when to stop and move on to the
next instruction? When the count of arguments reaches zero! This is why we store it on the (return)
stack: the interpreter will decrease this number by the count of consumed arguments until we reach
zero.
