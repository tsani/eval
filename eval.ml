  ...
  let print ppf : t -> unit = let open Format in function
    ...
    | ApplyNonClo (v, (sco, e)) ->
      fprintf ppf "@[<v>@[<v 2>Cannot apply non-closure@ %a@]@,@[<v 2>to argument@ %a@]@]"
        (Pretty.print_value 0) v
        (Pretty.print_tm 0 sco) e
    | InfiniteRecursion x ->
...
let rec match_pattern (env : Env.t) : Value.t * pattern -> Env.t option = function
  | Value.Num n1, NumPattern n2 -> if n1 = n2 then Some env else None
  | v, VariablePattern x -> Some Env.(extend env @@ alloc_entry ~contents: (Some v) x)
  | _, WildcardPattern -> Some env
  | Value.Const (ctor_name, spine), ConstPattern (pat_ctor_name, pat_spine) -> begin match () with
...
let rec eval (s : State.t) (env : Env.t) : Term.t -> Value.t = function
  | Num n -> Value.Num n
  | Var i -> begin match Env.lookup env i with
  ...
  | Fun (x, e) ->
    ...
    debug_print s "@[<hv 2>Construct closure@ %a@]@," (Pretty.print_value 0) v;
    v
  | App (e1, e2) as e ->
  ...
  let rec go = function
    ...
    | Case (pattern, body) :: cases -> match match_pattern env (scrutinee, pattern) with
      | Some env' ->
        ...
        Pretty.print_env env'
        (Pretty.print_tm 0 @@ Env.to_scope env') body;
        eval s env' body
      | None -> go cases
  in
  go cases
